#!/usr/bin/env python3
"""
HyMRPL — Adaptive class switch experiment (integrated engine).

Validates that the adaptive engine integrated in rpld correctly decides
the node's class based on 3 criteria:
  1. PDR (measured internally via DAO-ACK)
  2. Residual energy (read from /tmp/hymrpl_battery)
  3. Parent stability (internal parent change detection)

The test does NOT send FIFO commands — it only manipulates the conditions
and checks in the rpld logs whether the adaptive engine made the correct decision.
...
def apply_loss(sensor, loss_pct):
    """Apply packet loss on the sensor's link."""
    iface = get_iface_name(sensor)
...
def run_experiment(sensors, run_id):
    """
    6 phases that exercise the adaptive engine integrated in rpld.
    We manipulate external conditions and check whether rpld decides correctly.
    """
    info("\n{}\n=== ADAPTIVE SWITCH | Run {} ===\n{}\n".format("=" * 60, run_id, "=" * 60))
    results = {"run": run_id}
...
def print_summary(all_results):
    ...
    phases = ["A", "B", "C", "D", "E", "F"]
    descs = ["Stable bat=100%", "Bat=10%", "Bat=100% loss=30%",
             "Recovered", "Parent change", "Stabilized"]
